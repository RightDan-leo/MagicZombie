...
- 玩家进入游戏前需输入一个 2~32 个字符的玩家 ID，可使用中文或英文；系统会根据该 ID 绑定玩家等级、解锁关卡与最高分。
- 默认存档保存在浏览器，本地调试即可继续上次进度；部署环境可配置 Firebase（`VITE_FIREBASE_*`）实现跨设备同步。
- 允许通过访问链接 `/?profileId=<玩家ID>` 快速加载指定档案（用于自动化测试与分享）。
- 页面右上角提供“切换玩家”按钮；也可以在链接末尾加 `?resetProfile=1` 以强制重新输入玩家 ID。
- 登录完成后立即弹出“武器选择”面板，玩家可在闪电链 / 火焰喷射 / 水枪三种开局武器中选择一项，系统会记住该偏好并同步到云端。
- 武器经验与强化：击杀敌人时，根据武器类型获得经验；每当某武器升级，立即弹出 3 选 1 的武器增强界面（依据武器专属池随机），玩家可以叠加多次增强或选择跳过。
